...
[Notes: define "adaptive intervention", "optimal dynamic treatment policies",
...
Let's return to the lawn watering example to look at a SMART study design in practice. For this example, we will consider four simple adaptive policies that could be used to develop a dynamic treatment policy. Rather than looking at the daily weather, we will be adapting the amount of watering and the time of day that a lawn is watered, depending on a sample lawn's responsiveness to an initial treatment pattern. Let us define four adaptive interventions: 

- **Intervention A**: Never water. (Control, i.e. allow nature to naturally take care of the lawn.)
- **Intervention B**: Water every morning between 7--9am for half an hour.
- **Intervention C**: Water every evening between 7--9pm for half an hour.
- **Intervention D**: Water every morning between 7--9am for half an hour, **and** water every evening between 7--9pm for half an hour.

The outcome of interest for this trial is greenness of lawn. Greenness of a lawn is assessed on a 10-point scale with 1 being brown and 10 being deep green. Responsiveness to a treatment pattern is defined as follows:

- If the lawn is ranked < 6 at the start of an intervention, an increase of at least 2-points by the measurement point.
- If the lawn is ranked 6-8 at the start of an intervention, an increase of at least 1-point by the measurement point.
- If the lawn is ranked >8 at the start of an intervention, no decrease in greenness by the measurement point.

At the start of the trial, all lawns are assessed for greenness and randomly assigned to either **intervention A** or **intervention B** [stage 1]. After four weeks, all lawns are re-assessed for greenness. Any lawn that is assessed as responsive to the initial treatment (e.g. *responder*) is kept on the same intervention (**A** or **B**); any lawn that is assessed as a **non-responder** is randomly reassigned to either treatment **intervention C** or **intervention D** [stage 2]. All lawns are evaluated for greenness one month after the second randomization, allowing assessment of the following possible adaptive interventions:

- A --> responder --> A
- A --> non-responder --> C
- A --> non-responder --> D
- B --> responder --> B (control)
- B --> non-responder --> C
- B --> non-responder --> D

For this simple SMART design, we randomized at only two intervention points. It is easy to imagine extending this by adding additional interventions and re-randomizing based at responsiveness to treatment pattern at a more regular interval, say every two weeks for two months.

-----------------------------------------------

"lop[=[ Example: Lawn watering. Policy: Always on: every morning 9am water for 30 minutes Policy Andy: Surrendered to an intelligent system: Rachio listens to the weather service and deactivates if its going to rain. Policy Aimee: Designed so detects soil moisture + listens to the weather DAG: last two are dynamic and responsive to different characteristics. First is static Simulate data There is no possible optimal watering strategy Target outcome: greenness of grass. Design the ultimate strategy Now extend this: City planner is trying to see if the fatter sprinklers are better. He randomly assigns to the fatter sprinklers and designs a study areound the impact of the fatness. he notices Hector has his sprinkler off when it's raining and penalizes Hector: Anyone reactive is kicked out of the program. SMART trials are saying: we kind of get it. We're going to give you some combinations: Water every morning. Louise, water every third morning. But they are still not responsive to the weather -- variations on static assignment. SMART studies have limited ability to incorporate the weeather : the analysis may not even capture the weather and statistics would not be able to capture it. New strategist: if sunny, water. If rainy, don't water. Then we need analytic methods --- recover the rule from all the Trivial: categories of variables: e.g. color of the house. Or nutrients in the soil. Or what is downstream. What variety of grass (cate: conditional average treatment effects - we might identify a rule for different substrata. Different individualized treatment strategies based on actual traits of the yard and conditions.)(Listening should be different for different people)
