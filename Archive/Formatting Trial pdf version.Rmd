...
D_AW <- DAG.empty() + 
  node("age", distr = "rnorm", mean = 50, sd = 10) +
       ...
       node("priorchol", distr = "rnorm", mean = 200 + 0.5 * age, sd = 30) +
        node("statin", distr = "rbern", prob = plogis(-2 + 0.02 * priorchol + 
                                                        0.01 * age - 0.5 * gender)) +
        node("cholesterol", distr = "rnorm", mean = 180 - 20 * statin + 
               0.8 * priorchol + 0.5 * age + 5 * gender, sd = 15)
...
3)  **Regression:** Estimate the target parameter (e.g. ATE) by regressing the residual of the outcome variables onto the residuals of the treatment variables.
...
In the worked example, e're going to be using the same variables as before. So to remind and match with notation:
...
axis(1, at = c(1,2,3,4, 5,6), 
     labels = c("crude", "PS-match.", "PS-weight."," G-comp", "TMLE", "DML"))
axis(2)
...
**Aimee Harrison** holds a BS in Writing and Humanistic Studies from MIT and an MFA in Writing and Poetics from the University of Washington. Her early career was spent in academic publishing. She is currently a project manager at Navidence and head of product at Asclepia, both real-world data technology companies. She is interested in how we drive innovation and accessibility to real-world data tools through education.
