...
In this article, we will adopt some common notations from different authors who tend to not strictly adhere to conventions. Our primary goal is clarity and ease of understanding for the reader, even if this means diverging slightly from standardized notation.

### Notation conventions 

Some notation conventions we use:

- **A** and **T** to denote treatment variables.
...
Propensity score weighting is quite common in practice in epidemiology and public health and, as of this writing, is probably the most ubiquitous method for causal inference from observational data. 
...
(Note on notation: here, and in the next section on TMLE, as we mentioned in the introduction, we will sometimes use *W* and *X* interchangeably to refer to the set of confounding variables. In our worked example, both *X* and *W* will refer to the set of variables {Age, gender, and prior cholesterol})
...
We're going to be working with our same variables as before and so to remind and match with notation: 

- Y: Cholesterol
- T: Statin (treatment)
- **X**: Set of confounders: {age, gender, priorchol}
...
Despite their differences, all these methods aim to estimate the causal effect as accurately and *unbiasedly* as possible. They start with the same input data — observations of treatments, outcomes, and DAG-identified confounding variables — and apply different techniques to adjust for (remove) confounding to recover the causal parameter. (and remember, all associations draw their causal legitimacy from the **Identifiability assumptions** being met, more than the specific analytic estimation approach applied.)

We'll leave you with a summary of the strengths and weaknesses of these methods and invite you to read more, or contact the authors with any questions (or suggestions to improve the article). 
...
-   **Propensity score weighting:** As with PS-matching, the propensity score calculation can be done with best available machine learning models to estimate $\pi = e(T|X)$. 
