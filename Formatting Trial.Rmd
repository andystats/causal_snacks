...
```{r DAG0, echo=FALSE, fig.cap='Figure 1: Example of a simple DAG where treatment (statins) affects (causes or influences) outcome (high cholestoral).', out.width = '90%'}
...
# Bias and apparent (naive) average treatment effect {.tabset .tabset-fade .tabset-pills}

## Conceptual overview

Bias refers to systematic errors (e.g., confounders) that distort relationship between variables, leading to incorrect conclusions about causality. It is defined as $$bias = true - estimated$$. An estimator is said to be unbiased if its bias is zero. 
...
Comparing apparent (naive) estimates of the ATE and true ATE (using bias and percent bias), allows for understanding of the relative error introduced by confounders in the estimation process. In building your toolkit for effects measures, it is important to calculate the apparent (naive) estimate of ATE, bias, and percent bias, in order to assess how effective and reliable different methods are at understanding and correcting for biases. 
...
and percent bias is defined as:
...
Let's take a look at the apparent (naive) estimate of average treatment effect in our cholestoral example. We will compare this to the *true* ATE (a value we can only calculate because our synthetic data generated Y(1) and Y(0)).
