...
We want to pause and make an important distinction: $E[Y|A = 1] - E[Y|A = 0]$ is not equivalent to $E[Y(1)] - E[Y(0)]$. The former represents the difference of outcomes for actual treatments, which includes a lot of bias. The second represents the difference of outcomes for counterfactual treatments, in which bias has been corrected.

Let's consider an example where a benevolent CEO wants to understand the role of salary in happiness for all of his employees. Let's say all of his employees fit into two pay bands: one is considered high, one is considered low, so we can model pay as $A=1$ (high pay band) or $A=0$ (low pay band).

He conducts a survey where he asks all the employees to rank their happiness on a scale of 1-100 (observed $Y$). We can compare the average observed happiness within each pay band, and we get the values $E[Y|A=1]$ (high pay band) and $E[Y|A=0]$.

Now let's say he compares the two averages and determines he needs to intervene. The CEO is benevolent, but he is also random. He takes 75% and imposes a huge salary increase across the low pay band. . . [ANDY: you lost me here – can you fill in the gap to get from here to the E[Y(1) and E[Y(0)]???]

This represents $E[Y(1)]$ and $E[Y(0)]$
...
Observed outcome and counterfactual outcome for treated individuals ($T=1$) are represented by: $$Y, {Y}(0)$$ where $Y$ represents the observed outcome and $Y(0)$ represents the counterfactual treatment had they not received treatment ($T=0$).
