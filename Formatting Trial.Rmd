...
All the following libraries are available via [CRAN](https://cran.r-project.org/ "The Comprehensive R Archive Network (CRAN)") and can be installed either through the `tools` menu or using the `install.packages(" ")` command.
...
$$Y_t \perp\!\!\!\perp T \mid W$$ where the potential outcome $Y_t$ is independent of the treatment $T$ given the covariates $W$.
...
where a subject who received treatment $t$ has an observed outcome $Y_t$.

**No Interference** *(the treatment of one subject does not affect the outcome of another subject)* is represented as:

$$Y_{i,t} \text{ not affected by } T_j \text{ for } i \neq j$$.

or, in other words, the potential outcome $Y_{i,t}$ of subject $i$ is only affected by their own treatment $T_i$, not by the treatment of any other subject $T_j$.
...
Notice the value we put in above: `- 20 * statin` $\rightarrow$ this is where we seed in our causal effect that we'll recover later.
...
*Note that by convention both A and T can be used to represent treatment, though A is typically used for continuous or categorical treatment, and T is often used to indicate a binary treatment.*
...
Stabilized weights are used if any covariates are under- or overrepresented in either treatment group, which causes disproportionate representation of individuals, leading to high variance in the propensity score weights. Stabilized weights are calculated with an additional factor: the marginal probability of treatment (a measure of the overall probability of receiving a treatment across the entire study population).
...
In our opinion, targeted learning, and TMLE, offer the most promising method to debias target estimates and accurately estimate the true (treatment) effect from a given study design.
...
-   **Double robustness:** TMLE provides valid estimates even if either the outcome model or the propensity score model is mis-specified (but not both).
...
$$Q(T,X) = E[Y∣T,X]$$ where $E$ represents the original outcome regression model, $T$ is treatment value, and $X$ is a set of covariates.
...
where $\Psi$ is ATE, $w_i$ is the covariates of interest, and $1$ or $0$ represent the treated and untreated groups.
...
-   **Summary:** Uses propensity scores to standardize to the impact of inidividuals on average treatment effect based on covariates.
...
Despite our belief that machine learning algorithms are vital tools to make meaningful causal inferences from increasingly important real world data sets, we acknowledge a salient concern surrounding machine learning: that is problematic to trust and apply any algorithm without understanding its underlying mechanisms – particularly within pharmaceutical applications. One of our goals in offering this article is to ask you to take the first step into demystifying machine learning processes, so that our industry can better understand and implement this invaluable resource.

Let's take a look at the places where machine learning algorithms were used in the methods we covered today:

-   **Propensity score matching:** Andy: XX?

-   **Propensity score weighting:** Andy: XX?

-   **G-computation:** Ensemble machine learning algorithms (`SuperLearner` package) used to predict counterfactuals.

-   **TMLE:** Ensemble machine learning algorithms (`SuperLearner` package) used to predict counterfactuals and to determine the *fluctuation parameter*.

-   **DML:** High-performing machine learning tools (e.g., [random forest](https://www.ibm.com/topics/random-forest "Random forest is a machine learning algorithm that use multiple decision making trees to reach a single result. Read more about it on IBM's site, linked here.")) used to predict outcome and treatment variables.
...
**Aimee Harrison** holds a BS in Writing and Humanistic Studies from MIT and an MFA in Writing and Poetics from the University of Washington. Her early career was spent in academic publishing, and she is currently a project manager at Navidence and head of product at Asclepia, both real-world data technology companies. She is interested in how we drive innovation and accessibility to real-world data tools through education.

------------------------------------------------------------------------

# References

<!-- AH: still to fill out with all linked articles as well once we have a final set of links. -->
...
6.  Parikh, Harsh, Carlos Varjao, Louise Xu, and Eric Tchetgen Tchetgen. "Validating causal inference methods." In International conference on machine learning, pp. 17346-17358. PMLR, 2022.
