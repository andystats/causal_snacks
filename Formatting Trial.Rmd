...
All the following libraries are available via [CRAN](https://cran.r-project.org/ "The Comprehensive R Archive Network (CRAN)") and can be installed either through the `tools` menu or using `install.packages(" ")` command for each.
...
In an ideal research setting, we would be able to observe the outcome for a group of individuals with perfectly identical covariates under treated and untreated conditions. Because this is never possible in the real world, we rely on methods to ensure treated and untreated groups are as similar to one another as we can make them. To establish the *legitimacy* of causal claims, we must satisfy [causal idenifiability assumptions](https://www.hsph.harvard.edu/miguel-hernan/wp-content/uploads/sites/1268/2024/01/hernanrobins_WhatIf_2jan24.pdf "Causal identifiability assumptions provide the set of conditions that allow us to estimate effects measures. For a more in-depth exploration of causal identifiability assumptions, see chapter three in Hernán and Robins (2010), Causal Inference: What If"). These are, briefly:
...
where a subject who received treatment $t$ has an observed outcome is $Y_t$.
...
Notice the value we put in above: `- 20 * statin` $\rightarrow$ this is where we seed in our causal effect we'll recover later.
...
Aimee Harrison holds a BS in Writing and Humanistic Studies from MIT and an MFA in Writing and Poetics from the University of Washington. Her early career was spent in academic publishing, and she is currently a product manager at Navidence and head of product at Asclepia, both real-world data technology companies. She is interested in how we drive innovations in real-world data tools through access and education. <!-- My title at Nav. is in flux, and I was told last week to make my own title so let me confirm these with Amanda before we publish. -->
